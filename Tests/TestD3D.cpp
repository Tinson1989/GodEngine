...
void CreateRenderTarget() {
    ...
    //这个RenderTarget 绑定到渲染管线Output Merge 阶段的输出目标
    g_pDevcon->OMSetRenderTargets( 1, &g_pRTView, nullptr );
}
//设置 RenderTarget 到windows窗口的的对应关系
void SetViewPort() {
...
HRESULT CreateGraphicsResources(HWND hWnd)
{
    ...
    if (g_pSwapchain == nullptr)
    {
        ...
        hr = D3D11CreateDeviceAndSwapChain(NULL,
                                           ...
                                           &g_pDevcon);
        //这个函数调用了两次，目的是防止当前硬件设备不支持声明的 FeatureLevels，于是会在失败的情况下使用返回的支持的FeatureLevelSupported 。
        if (hr == E_INVALIDARG) {
            hr = D3D11CreateDeviceAndSwapChain(NULL,
                                               D3D_DRIVER_TYPE_HARDWARE,
                                               NULL,
